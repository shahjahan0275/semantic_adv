...
def set_denoiser(checkpoint_path, cfg):
    ...
    if cfg['gpu'] == 'cpu':
        ...
        #model = {key: module[key].to("cpu") for key in module}
...
#Full CPU-ready snippet for set_denoiser()
'''
def set_denoiser(checkpoint_path, cfg):
    module = set_module(cfg)
    load_checkpoint(module, cfg, checkpoint_path)
    ...
    # get the denoiser function
    denoiser_base = getattr(model['denoiser'], 'module', model['denoiser'])
    denoiser = getattr(denoiser_base, 'denoise', denoiser_base)
    return denoiser
'''
...
def load_checkpoint(module, cfg, checkpoint_path):
    ...
    # 🔑 Force map everything to CPU if CUDA is not available
    if not torch.cuda.is_available() or cfg.get("gpu", "cpu") == "cpu":
        map_location = torch.device("cpu")
    else:
        map_location = None

    # this ensures CUDA checkpoints can still be loaded on CPU-only systems
    saved_checkpoint = torch.load(file_name, map_location=map_location)

    for key in module:
        if "model_weight" in saved_checkpoint and key in saved_checkpoint["model_weight"]:
            module[key].load_state_dict(saved_checkpoint["model_weight"][key], strict=False)
        else:
            print(f"[load_checkpoint] Warning: {key} not found in checkpoint")
'''
