...
class FourthOrderPooling(nn.Module):
    """
    Compute fourth-order (kurtosis-like) statistics.
    Input: (B, N, D)
    Output: (B, output_dim)
    """
    def __init__(self, input_dim, output_dim=None, eps=1e-5):
    ...
    def forward(self, x):
        ...
        outer4 = (xc.unsqueeze(3).unsqueeze(4).unsqueeze(5) *
                  xc.unsqueeze(2).unsqueeze(4).unsqueeze(5) *
                  xc.unsqueeze(2).unsqueeze(3).unsqueeze(5) *
                  xc.unsqueeze(2).unsqueeze(3).unsqueeze(4))  # (B, N, D,D,D,D)
        fourth = outer4.mean(dim=1)
        fourth_flat = fourth.reshape(B, -1)

        if self.proj is not None:
            fourth_flat = self.proj(fourth_flat)
        return fourth_flat
'''
class FourthOrderPooling(nn.Module):
    """
