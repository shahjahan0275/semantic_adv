...
if __name__ == '__main__':
    ...
    ##This part has been changed incase the training is interrupted in h100-dell##
    start_epoch = 0

    if opt.resume_path is not None:
        #start_epoch = model.load_networks(opt.resume_path)
        # FIX: override because old checkpoint stored total_steps
        start_epoch = 28   # manually set correct last epoch
        print(f"Resuming training from epoch {start_epoch}")
        # optional: recalc total_steps
        model.total_steps = (start_epoch + 1) * len(data_loader)
    ##This part has been changed incase the training is interrupted in h100-dell##
    ...
    ##This line below has been changed incase the training is interrupted in h100-dell##
    for epoch in range(start_epoch + 1, opt.niter):
        ...
        if epoch % opt.save_epoch_freq == 0:
            ...
            model.save_networks('model_epoch_best.pth', epoch)
            model.save_networks(f'model_epoch_{epoch}.pth', epoch)
