...
class CLIPModelShuffleAttentionPenultimateLayer(nn.Module):
    ...
    def forward(self, x, return_feature=False):
        ...
        # Self-Attention over stacked [e_o, e_s]
        features = self.attention_head(torch.stack(features, dim=-2))
        
        # --- REPLACEMENT ---
        return features
        #return self.fc(attn_out)   # robust MLP classifier
    '''
    # -------------------------------
    ...
    def forward(self, x, return_feature=False):
        ...
        with torch.no_grad():
            ...
            features.append(noise_feat)
        # Self-Attention across [e_o, e_s, noise][original, patch-shuffle, noise]
        features = self.attention_head(torch.stack(features, dim=-2))
        return features
