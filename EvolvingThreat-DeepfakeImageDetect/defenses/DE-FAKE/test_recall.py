...
fake_test = CustomFakeDataset(
    datafile="/speed-scratch/a_shahj/EvolvingThreat-DeepfakeImageDetect/defenses/DE-FAKE/csv_outputs/test_fake.csv"
)

# Only fake samples
test_dataset = fake_test
...
# Only recall is valid when dataset contains only fake (label = 1)
if (tp + fn) > 0:
    recall = tp / (tp + fn)
else:
    recall = 0.0
