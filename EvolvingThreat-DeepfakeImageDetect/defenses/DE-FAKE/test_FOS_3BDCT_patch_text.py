...
#DCT spectral statistics (576)
#TOTAL = 1600-dimensional input
# ============================================================
# TEST SCRIPT — MATCHED 100% TO TRAINING (IMAGE + TEXT + DCT → 1600-D)
# ============================================================
...
# ---------------------------------------------------------
# 5-LAYER MLP EXACTLY MATCHING TRAINING
# ---------------------------------------------------------
class NeuralNet(nn.Module):
    def __init__(self, input_size=1600, num_classes=2, dropout_p=0.5):
        super().__init__()
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, self.label, caption, dct_feat
...
linear = NeuralNet(input_size=1600, num_classes=2).to(device)

# Load CLIP state_dict
...
# Load Linear MLP
linear.load_state_dict(torch.load(args.outputpath_linear, map_location=device))
...
test_loader = DataLoader(
    ConcatDataset([real_test, fake_test]),
    batch_size=128,
    shuffle=False,
    num_workers=4
)
...
# ---------------------------------------------------------
# INFERENCE LOOP — EXACT MATCH TO TRAINING PIPELINE
# ---------------------------------------------------------
all_preds = []
all_true = []

with torch.no_grad():
    for x, y, captions, d in tqdm(test_loader):
        x = x.to(device)
        y = y.to(device)
        d = d.to(device)

        # Encode captions the same way as training
        text_tokens = clip.tokenize(list(captions), context_length=77).to(device)
        text_emb = model.encode_text(text_tokens)     # (B,512)
        img_emb  = model.encode_image(x)              # (B,512)

        emb = torch.cat((img_emb, text_emb, d), dim=1)  # 1600-D
