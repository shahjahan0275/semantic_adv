#Patch-wise FFT-based 4th-order spectral statistics
...
def compute_patch_fft_stats(img, num_patches=8):
    """
    img: (3, 224, 224)

    Bands: LOW, MID, HIGH
    Per patch = 3×4×3 = 36 dims
    64 patches = 2304 dims
    """
    ...
    bands = [LOW, MID, HIGH]   # <-- ONLY 3 BANDS
    ...
    for py in range(num_patches):
        for px in range(num_patches):
            ...
            patch_stats = torch.cat(patch_stats, dim=1).reshape(-1)
            features.append(patch_stats)

    # 64 patches × 36 dims = 2304
...
class CustomRealDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        fft_feat = compute_patch_fft_stats(image)   
...
class CustomFakeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        image = self.transform(image).float()
        #return image, label, caption
        fft_feat = compute_patch_fft_stats(image)   # 576-dim patch-based FFT stats
        return image, label, "", fft_feat
...
# Correct input size:
# CLIP: 512 dims
# FFT stats: 2304 dims (64 patches × 36 features)
# TOTAL = 3328
# --------------------------------------
linear = NeuralNet(
    input_size=2816,
