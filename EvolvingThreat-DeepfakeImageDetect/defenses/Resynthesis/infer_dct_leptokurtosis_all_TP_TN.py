...
def test(test_loader, model, sr_model, perception_net, dct_extractor, args):
    ...
    # Removed counters (viz_count_fake, max_viz) to print ALL TP/TN images
    ...
    for i, (input, target, post_path) in enumerate(test_loader):     
        ...
        pred_val = (output[:, 0] < output[:, 1]).cpu().numpy()[0] # True if Fake, False if Real
        ...
        # Determine True Positive (Correctly detected Fake) and True Negative (Correctly detected Real)
        ...
        # Condition: Print ALL TP and TN images
        if is_tp or is_tn:
            ...
            gt_label = 'Fake' if is_fake else 'Real'
            pred_label = 'Fake' if is_pred_fake else 'Real'
            res_type = "TP" if is_tp else "TN"
            
            plt.title(f"[{res_type}] {gt_label} Residual\nGT: {gt_label}, Pred: {pred_label}")
            ...
            # Save file with TP/TN prefix
            plt.savefig(f"{viz_dir}/{prefix}{res_type}_{'fake' if is_fake else 'real'}_{i}.pdf", bbox_inches='tight')
            plt.close()
