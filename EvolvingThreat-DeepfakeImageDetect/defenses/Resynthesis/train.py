...
class DCTStatExtractor(nn.Module):
    """
    Computes 4th-order statistics of the 2D DCT:
         mean, variance, skewness, kurtosis
    Produces a vector of shape [B, 4*C]
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):  
        ...
        stats = torch.cat([mean, var, skew, kurt], dim=1)   # [B, 4*C]
        return stats
'''
...
def train(epoch, test_loader, model, sr_model, perception_net, optimizer, criterion, criterion2, args):
    ...
    for i, (input, target, post_path) in enumerate(test_loader):     
        ...
        if args.idx_stages > 0:
            ...
            rec_features = torch.abs(per_rec[args.idx_stages - 1] - per_gt[args.idx_stages - 1])
        else:
            rec_features = torch.abs(preds_input - input)  # [B, C, H, W]

        # ---- Compute DCT statistics ----
        dct_extractor = DCTStatExtractor().cuda(args.gpu)
        with torch.no_grad():
            dct_stats = dct_extractor(rec_features)  # shape [B, 4*C]

        # ---- Ensure shape matches model expectation ----
        if dct_stats.shape[1] != 4 * args.input_channel:
            raise RuntimeError(f"DCT stats shape mismatch! Got {dct_stats.shape}, expected [B, {4*args.input_channel}]")

        # ---- Forward pass with DCT stats ----
        output, aa = model(rec_features, dct_stats)
