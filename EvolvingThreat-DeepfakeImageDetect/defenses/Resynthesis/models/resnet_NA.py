...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, input_channel=3, num_classes=1000, dilated=True, multi_grid=False,
        ...
        self.fc = nn.Linear(512 * block.expansion + 4 * input_channel, num_classes)
    ...
    def forward(self, x, dct_stats):
        ...
        # ---- Global average pool ----
        feats = self.avgpool(x2)
        feats = feats.view(feats.size(0), -1)   # (B, 2048) for ResNet50

        # ---- DCT stats MUST be provided ----
        if dct_stats is None:
            raise RuntimeError("DCT stats missing: model expects (B, 4*input_channel)")

        # Check shape correctness
        expected = 4 * self.input_channel
        if dct_stats.shape[1] != expected:
            raise RuntimeError(
                f"DCT stats wrong shape: expected {expected}, got {dct_stats.shape[1]}"
            )

        # ---- Append DCT stats ----
        feats = torch.cat([feats, dct_stats], dim=1)
