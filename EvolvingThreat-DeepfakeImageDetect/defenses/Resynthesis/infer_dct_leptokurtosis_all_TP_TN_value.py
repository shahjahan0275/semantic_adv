...
def test(test_loader, model, sr_model, perception_net, dct_extractor, args):
    ...
    # Removed limits to print ALL TP and TN
    ...
    for i, (input, target, post_path) in enumerate(test_loader):     
        ...
        # Only process TP or TN
        if is_tp or is_tn:
            
            # --- CSV WRITING ---
            img_name = os.path.basename(post_path[0]) if isinstance(post_path, (list, tuple)) else str(i)
