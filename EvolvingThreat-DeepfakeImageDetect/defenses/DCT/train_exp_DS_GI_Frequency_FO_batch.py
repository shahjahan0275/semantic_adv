...
def load_balanced_data(base_dir, num_real, num_fake, seed=42, grayscale=True, input_size=1024):
    ...
    generators = [ "Midjourney", "stable_diffusion_v_1_4", "stable_diffusion_v_1_5", "wukong"]
...
def spectral_features(img_batch, patch=32):
        ...
        def compute_stats(mask):
            ...
            # skewness
            skew = (((xc**3).sum(dim=[2, 3]) / cnt) /
                    (var.sqrt()**3 + eps))

            # kurtosis
            kurt = (((xc**4).sum(dim=[2, 3]) / cnt) /
                    (var**2 + eps))

            stats = torch.stack([mean, var, skew, kurt], dim=-1)
            return stats

        low_stats  = compute_stats(low_mask)   # [B,N,4]
        mid_stats  = compute_stats(mid_mask)   # [B,N,4]
        high_stats = compute_stats(high_mask)  # [B,N,4]
...
def spectral_features_batched(x, batch_size=4):
    all_feats = []
    n = x.shape[0]

    for i in tqdm(range(0, n, batch_size), desc="DCT Mini-Batches", ncols=100, colour="cyan"):
        batch = x[i:i+batch_size].to("cpu")  # keep on CPU to prevent GPU OOM
        feat = spectral_features(batch)      # your original function
...
def main(args):
    ...
    print("\nExtracting TRAIN features safely in mini-batches...")
    x_train_tf = spectral_features_batched(x_train, batch_size=4)

    print("\nExtracting VAL features safely in mini-batches...")
    x_val_tf = spectral_features_batched(x_val, batch_size=4)
