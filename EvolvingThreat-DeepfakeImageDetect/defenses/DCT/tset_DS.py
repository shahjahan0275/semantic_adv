...
def main(args):
    ...
    # ✅ Infer input size directly from training mean shape
    if means.ndim == 2:
    ...
    def process_dir(imgdir, label):
        ...
        for p in paths:
            ...
            # ✅ Align feature length with training stats
            train_feat_len = input_size
            test_feat_len = x_tf.shape[1]

            if test_feat_len > train_feat_len:
                x_tf = x_tf[:, :train_feat_len]
            elif test_feat_len < train_feat_len:
                pad_len = train_feat_len - test_feat_len
                x_tf = torch.nn.functional.pad(x_tf, (0, pad_len))
    ...
    result_path = os.path.join(result_dir, "RAID_results.csv")
