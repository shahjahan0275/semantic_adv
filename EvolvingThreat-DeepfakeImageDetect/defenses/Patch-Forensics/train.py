...
def train(opt):
    ...
    while True:
        ...
        for i, ims in enumerate(dl):
            ims_real = ims['original'].to(opt.gpu_ids[0]).contiguous()
            ims_fake = ims['manipulated'].to(opt.gpu_ids[0]).contiguous()
            ...
            batch_im = torch.cat((ims_real, ims_fake), dim=0).contiguous()
...
def validate(model, opt):
    ...
    for i, ims in enumerate(val_dl):
        ...
        inputs = dict(ims=torch.cat((ims_real, ims_fake), dim=0).contiguous(),
