...
def get_transform(opt, for_val=False):
    ...
    # 🚫 NO RESIZE
    # 🚫 NO CROP
    # Images stay at original 1024 × 1024
    # ============================================================

    if not for_val:
        # ----- SAFE augmentations (do NOT change geometry) -----

        if opt.cnn_detection_augment:
            transform_list.append(
                CNNDetectionAugmentations(opt.cnn_detection_augment)
            )
        ...
        # Horizontal flip is geometry-preserving (optional)
        if not opt.no_flip:
            transform_list.append(
...
class PatchShuffle(object):
    ...
    def __call__(self, image):
        ...
        # 🔒 PAD instead of CROP (CRITICAL FIX)
        pad_h = (ps - H % ps) % ps
        pad_w = (ps - W % ps) % ps
