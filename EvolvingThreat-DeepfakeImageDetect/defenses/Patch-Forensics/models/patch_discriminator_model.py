...
class PatchDiscriminatorModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        # ✅ OPTIONAL DCT (only if provided)
        self.dct = input.get('dct', None)
        if self.dct is not None:
            self.dct = self.dct.to(self.device)

    def forward(self):
        # ✅ Only ResNet-based discriminators support DCT
        if (
            self.dct is not None
            and isinstance(self.opt.which_model_netD, str)
            and self.opt.which_model_netD.startswith('resnet')
        ):
            self.pred_logit = self.net_D(self.ims, self.dct)
        else:
            # Xception & others
