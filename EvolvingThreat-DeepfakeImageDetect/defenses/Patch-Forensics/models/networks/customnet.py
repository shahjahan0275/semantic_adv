...
class PatchResNetWithDCT(nn.Module):
    """
    Patch discriminator ResNet + global DCT stats
    """
    def __init__(self, backbone, num_classes=2, dct_dim=12):
        super().__init__()

        self.backbone = backbone  # outputs [B,C,H,W]
        self.dct_proj = nn.Linear(dct_dim, backbone.out_channels)

        self.conv_out = nn.Conv2d(
            backbone.out_channels * 2, num_classes, kernel_size=1
        )

    def forward(self, x, dct):
        feat = self.backbone(x)          # [B,C,H,W]

        # project & broadcast DCT
        dct_feat = self.dct_proj(dct)    # [B,C]
        dct_feat = dct_feat[:, :, None, None]
        dct_feat = dct_feat.expand_as(feat)

        fused = torch.cat([feat, dct_feat], dim=1)
        return self.conv_out(fused)


def make_patch_resnet(depth, layername, num_classes=2, use_dct=False):
    def change_out(layers):
        ...
        layers[ind+1:] = []  # remove FC
        return layers, num_ch

    model = CustomResNet(depth, modify_sequence=lambda x: x)
    layers = list(model._modules.items())
    layers, out_ch = change_out(layers)

    backbone = nn.Sequential(OrderedDict(layers))
    backbone.out_channels = out_ch

    if use_dct:
        return PatchResNetWithDCT(backbone, num_classes)
    else:
